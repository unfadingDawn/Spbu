...
namespace multithread_container {
    void HashMap::put(HashMap& map, const int hash, const BaseValue& value) {
        map.buckets_[map.hash_func(hash)].add(Value(value, hash));
    }
    void HashMap::remove(HashMap& map, const int hash) {
        map.buckets_[map.hash_func(hash)].remove(hash);
    }

    bool HashMap::check(HashMap &map, const int hash) {
